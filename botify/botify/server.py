...
catalog_mf = Catalog(app).load(
    app.config["TRACKS_CATALOG_MF"], app.config["TOP_TRACKS_CATALOG"], app.config["TRACKS_WITH_DIVERSE_RECS_CATALOG"]
)
catalog_mf.upload_tracks(tracks_redis.connection, tracks_with_diverse_recs_redis.connection)
catalog_mf.upload_artists(artists_redis.connection)
catalog_mf.upload_recommendations(recommendations_redis.connection)
catalog_mf.upload_recommendations(recommendations_redis.connection, "RECOMMENDATIONS_FILE_PATH")
...
class NextTrack(Resource):
    def post(self, user: int):
        ...
        if treatment == Treatment.T1:
            recommender = MFContextualRecommender(tracks_redis, recommendations_redis, catalog_mf)
        else:
